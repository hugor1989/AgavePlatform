...
import { Search, MapPin, Eye, Building2, CheckCircle, XCircle } from "lucide-react"
...
export default function FarmerOffersPage() {
  ...
  const [confirmingOffer, setConfirmingOffer] = useState<{ id: number; action: "aceptada" | "rechazada" } | null>(null)
  ...
  if (loading) return <AppLayout type="farmer"><p className="p-4">Cargando ofertas...</p></AppLayout>
      ...
      <div className="space-y-6">
        ...
        {filtered.length === 0 ? (
          ...
          <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-3">
            {filtered.map((group) => (
              <Card key={group.orchardId} className="overflow-hidden hover:shadow-lg transition-shadow">
                <div className="relative">
                  <Image
                    src={orchardService.getPhotoUrl(group.coverPhoto) || "/agave-field-plantation.png"}
                ...
                <CardContent className="p-4 space-y-3">
                  <div>
                    <h3 className="font-semibold text-lg text-gray-900">{group.orchardName}</h3>
                  ...
                  <div className="flex items-center gap-2 pb-3 border-b border-gray-100">
                    ...
                    <span className="text-sm font-medium text-gray-900">{group.agaveType}</span>
                  </div>

                  <div className="text-center py-2">
                    <p className="text-sm text-gray-500 mb-1">Cantidad de Plantas</p>
                    <span className="text-2xl font-bold text-blue-600">{group.plantQuantity.toLocaleString()}</span>
                  </div>
                  ...
                  <div className="grid grid-cols-2 gap-4">
                    <div className="flex items-center gap-2">
                      ...
                      <div>
                        ...
                        <p className="text-sm font-medium text-gray-900">{group.state}</p>
                      </div>
                    </div>
                    <div className="flex items-start gap-2">
                      <div>
                        <p className="text-sm text-gray-500">Municipio</p>
                        <p className="text-sm font-medium text-gray-900">{group.municipality}</p>
                      </div>
                    </div>
                  </div>
                  ...
                  <div className="bg-orange-50 border border-orange-200 rounded-lg p-3 shadow-sm">
                    <div className="flex items-center justify-between">
                      ...
                      <span className="text-lg font-bold text-orange-800">{group.offers.length}</span>
                    </div>
                    {(() => {
                      const ready    = group.offers.filter(o => o.status === "revisada").length
                      const pending  = group.offers.filter(o => o.status === "pendiente").length
                      const accepted = group.offers.filter(o => o.status === "aceptada").length
                      const rejected = group.offers.filter(o => o.status === "rechazada").length
                      return (
                        <div className="flex gap-2 mt-2 flex-wrap">
                          {ready    > 0 && <span className="text-xs bg-blue-100 text-blue-700 px-2 py-0.5 rounded-full">{ready} lista{ready > 1 ? "s" : ""}</span>}
                          {pending  > 0 && <span className="text-xs bg-yellow-100 text-yellow-700 px-2 py-0.5 rounded-full">{pending} en revisión</span>}
                          {accepted > 0 && <span className="text-xs bg-green-100 text-green-700 px-2 py-0.5 rounded-full">{accepted} aceptada{accepted > 1 ? "s" : ""}</span>}
                          {rejected > 0 && <span className="text-xs bg-red-100 text-red-700 px-2 py-0.5 rounded-full">{rejected} rechazada{rejected > 1 ? "s" : ""}</span>}
                        </div>
                      )
                    })()}
                  </div>

                  <Button className="w-full bg-teal-600 hover:bg-teal-700" onClick={() => handleViewOffers(group)}>
          ...
          <DialogContent className="max-w-4xl max-h-[80vh] overflow-y-auto">
            <DialogHeader>
              ...
              <p className="text-sm text-muted-foreground">
                ...
                {selectedGroup?.offers.length === 1 ? "oferta recibida" : "ofertas recibidas"}
              </p>
            </DialogHeader>
                      ...
                      <div className="flex items-center gap-3">
                        ...
                        <div>
                          <h3 className="font-semibold text-lg text-gray-900">
                            {offer.company?.business_name ?? `Empresa #${offer.company_id}`}
                          </h3>
                          <p className="text-sm text-gray-500">
                            Oferta #{offer.id} · {new Date(offer.created_at).toLocaleDateString("es-MX")}
                          </p>
                        </div>
                      </div>
                    ...
                    {offer.status === "pendiente" ? (
                      ...
                      <div className="bg-yellow-50 border border-yellow-200 rounded-lg p-5 text-center space-y-2">
                        ...
                        <p className="text-xs text-yellow-600">
                          El administrador está revisando la oferta. Cuando esté lista verás todos los detalles y tu precio de venta.
                        </p>
                      </div>
                    ) : (
                      ...
                      <div className="space-y-4">
                        ...
                        {offer.farmer_price !== null && (
                          <div className="bg-blue-50 border border-blue-200 rounded-lg p-4">
                            <p className="text-sm font-semibold text-blue-800 mb-1">Tu precio de venta</p>
                            <p className="text-2xl font-bold text-blue-900">
                              ${Number(offer.farmer_price).toLocaleString("es-MX")}
                            </p>
                            {offer.admin_notes && (
                              <p className="text-xs text-blue-600 mt-2">{offer.admin_notes}</p>
                            )}
                          </div>
                        )}
                        ...
                        {offer.status === "revisada" && (
                          <div className="pt-2 border-t border-gray-100">
                            {confirmingOffer?.id === offer.id ? (
                              <div className={`rounded-lg p-4 space-y-3 ${confirmingOffer.action === "aceptada" ? "bg-green-50 border border-green-200" : "bg-red-50 border border-red-200"}`}>
                                ...
                                <div className="flex gap-2">
                                  <Button
                                    size="sm"
                                    className={confirmingOffer.action === "aceptada" ? "bg-green-600 hover:bg-green-700" : "bg-red-600 hover:bg-red-700"}
                                    disabled={actionLoading}
                                    onClick={() => executeOfferAction(offer.id, confirmingOffer.action)}
                                  >
                              ...
                              <div className="flex flex-col sm:flex-row gap-3">
                                <Button
                                  ...
                                  onClick={() => setConfirmingOffer({ id: offer.id, action: "aceptada" })}
                                >
                                ...
                                <Button
                                  ...
                                  onClick={() => setConfirmingOffer({ id: offer.id, action: "rechazada" })}
                                >
